...
let indexLast = currentPageNumber * PAGE_LIMIT; // index used to slice to based on the current page number
let indexFirst = indexLast - PAGE_LIMIT; // index used to slice to based on the page limit and the last index. 
const articlesWrapper = document.querySelector('.articles-wrapper'); // used to store posts
const formContainer = document.querySelector('.form-container') // form container used to transition from open to closed
const form = document.querySelector('form');
const pageNavItems = Array.from(document.querySelectorAll('.page-btn')); // array used to store page buttons to implement event listeners
const writeBtn = document.querySelector('.accent-btn'); 
const postBtn = document.querySelector('button[type=submit]'); // post button used to add a blog to the posts array
const closeBtn = document.querySelector('.close-btn'); // button used to close out from modal view 
const posts = [
    {
      "id": 0,
...
function pagination()
{
  ...
  if ((posts.length / PAGE_LIMIT) % 2 == 0)
    {
      page_total = parseInt((posts.length / PAGE_LIMIT));
    }
    else
    {
      page_total = parseInt((posts.length / PAGE_LIMIT)) + 1;
    }
  
  for (let i = 0; i < page_total - pageNavItems.length; i++)
  {
    const button = document.createElement('button');
    button.classList.add('page-btn');
    pageNavItems.push(button);
    button.textContent = pageNavItems.length;
    paginationContainer.appendChild(button);
  }

  for (let i = 0; i < pageNavItems.length; i++)
  {
    pageNavItems[i].addEventListener('click',change)
  }
}
...
function change(e)
{
  ...
  else
  {
    ...
    currentPageNumber = parseInt(document.querySelector('.page-btn.active').innerText);
    indexLast = currentPageNumber * PAGE_LIMIT;
    indexFirst = indexLast - PAGE_LIMIT;
    let temp = posts.slice(indexFirst, indexLast);
    addPosts(temp);
  }
}

function addPosts(articles)
{
  ...
  for (let i = 0; i < articles.length; i++)
  {
    ...
    card.setAttribute('data-id', articles[i].id);
    ...
    profile.src = articles[i].profile;
    profile.alt = 'profile picture';
    profile.classList.add('avatar');
    cardHeader.appendChild(profile);

    const author = document.createElement('div');
    author.textContent = `${articles[i].author} · ${new Date(articles[i].date).toDateString()}`;
    ...
    h3.textContent = articles[i].title;
    const p = document.createElement('p');

    if (articles[i].content.length > MAX_LENGTH)
    {
      let textHolder = [];
      textHolder[0] = articles[i].content.substring(0, MAX_LENGTH);
      textHolder[1] = articles[i].content.substring(MAX_LENGTH, articles[i].content.length);
      p.textContent = textHolder[0] + ' ...';
    }
    else
    {
      p.textContent = articles[i].content + String(" ...");
    }
...
function createPost(e)
{
  ...
  if (form.reportValidity())
  {
    ...
    const article = {id, title, author, date, profile, content};
    
    posts.unshift(article);
    formContainer.classList.remove('open');
    addPosts(posts.slice(indexFirst,indexLast));
    e.preventDefault();
  }
}
...
articlesWrapper.addEventListener('click', popUp);
writeBtn.addEventListener('click', show_hide_form);
postBtn.addEventListener('click', createPost);
window.addEventListener('DOMContentLoaded', addPosts(posts.slice(indexFirst,indexLast)));
